...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        .frame(minWidth: 720, idealWidth: 780, minHeight: 620, idealHeight: 720)
    }
}
        ...
        SettingsScrollView {
            ...
            VStack(spacing: 8) {
                if let appVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String {
                    Text("QuickRecorder Enhanced v\(appVersion)")
                        .font(.subheadline)
    ...
    var body: some View {
        SettingsScrollView {
            ...
            SGroupBox {
                ...
                SToggle("Automatically create AV1 archive after recording", isOn: $autoCreateAV1ArchiveAfterRecording, tips: "After the HEVC master is saved, QuickRecorder will create a verified AV1 archive in the background.")
                SDivider()
...
struct OutputView: View {
    ...
    @AppStorage("audioChannels")    private var audioChannels: AudioChannels = .stereo
    ...
    @AppStorage("microphoneNoiseReduction") private var microphoneNoiseReduction: Bool = false
    ...
    var body: some View {
        SettingsScrollView {
            SForm(spacing: 20, noSpacer: true) {
                SGroupBox(label: "Preset") {
                    SButton("Lecture HEVC Master + AV1 Auto Archive", buttonTitle: "Apply", tips: "Sets H.265 MP4, High 2x, 15 FPS, adaptive VFR, automatic video bitrate, AAC 64 Kbps mono audio, microphone noise reduction, and automatic AV1 archive after recording.") {
                        encoder = .h265
                        videoFormat = .mp4
                        highRes = 2
                        frameRate = 15
                        ...
                        enableAEC = false
                        autoCreateAV1ArchiveAfterRecording = true
                ...
                SGroupBox(label: "Audio") {
                    SPicker("Quality", selection: $audioQuality) {
                        ...
                        Text("Speech Low - 48Kbps").tag(AudioQuality.speechLow)
                        Text("Speech - 64Kbps").tag(AudioQuality.speech)
                        Text("Speech High - 96Kbps").tag(AudioQuality.speechHigh)
                    ...
                    SDivider()
                    SPicker("Channels", selection: $audioChannels) {
                        Text("Mono").tag(AudioChannels.mono)
                        Text("Stereo").tag(AudioChannels.stereo)
                    }
                    SDivider()
                    ...
                    SToggle("Microphone Noise Reduction", isOn: $microphoneNoiseReduction, tips: "Reduces steady background noise on the microphone using the tested 80% RNNoise and 20% original mix. System audio is not changed.")
                        .onChange(of: microphoneNoiseReduction) { enabled in
                            if enabled { enableAEC = false }
                        }
                    SDivider()
                    SToggle("Enable Acoustic Echo Cancellation", isOn: $enableAEC)
                        .onChange(of: enableAEC) { enabled in
                            if enabled { microphoneNoiseReduction = false }
                        }
                    if #available(macOS 14, *) {
                        SDivider()
                ...
                SGroupBox(label: "Video") {
                    ...
                    SDivider()
                    SPicker("Bitrate", selection: $videoBitrate, tips: "Auto keeps QuickRecorder's original quality-based bitrate. Fixed low bitrates are useful for lecture slides and mostly static screens.") {
                        Text("Auto").tag(VideoBitrate.auto)
                        Text("50 Kbps").tag(VideoBitrate.kbps50)
                        Text("75 Kbps").tag(VideoBitrate.kbps75)
                        Text("100 Kbps").tag(VideoBitrate.kbps100)
                        Text("150 Kbps").tag(VideoBitrate.kbps150)
                        Text("200 Kbps").tag(VideoBitrate.kbps200)
                        Text("300 Kbps").tag(VideoBitrate.kbps300)
                        Text("500 Kbps").tag(VideoBitrate.kbps500)
                        Text("800 Kbps").tag(VideoBitrate.kbps800)
                        Text("1000 Kbps").tag(VideoBitrate.kbps1000)
                    }.disabled(withAlpha)
                    SDivider()
                    SToggle("Adaptive VFR", isOn: $adaptiveVFR, tips: "Skips tiny-change frames while preserving full-rate frames during active motion. Best for lecture slides, notes, and mostly static screens.")
                        .disabled(withAlpha || recordHDR)
                    SDivider()
