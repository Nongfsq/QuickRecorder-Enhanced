                    ...
                    Button(action: {
                        ...
                        SCContext.updateAvailableContent {
                            DispatchQueue.main.async {
                                appDelegate.startAreaSelectorWithDisplayTracking()
                            }
                        }
                    }, label: {
...
extension AppDelegate {
    func closeAreaSelector() {
        if isClosingAreaSelector { return }
        isClosingAreaSelector = true
        defer { isClosingAreaSelector = false }

        for window in NSApp.windows.filter({
            $0.title == "Area Selector".local ||
            $0.title == "Start Recording".local
        }) {
            window.close()
        }
        ...
        if let monitor = areaSelectorMouseMonitor {
            NSEvent.removeMonitor(monitor)
            areaSelectorMouseMonitor = nil
        }

    }

    func startAreaSelectorWithDisplayTracking(size: NSSize = NSSize(width: 600, height: 450)) {
        showAreaSelector(size: size)
        ...
        areaSelectorMouseMonitor = NSEvent.addGlobalMonitorForEvents(matching: [.mouseMoved, .rightMouseDown, .leftMouseDown, .otherMouseDown]) { [weak self] _ in
            let display = SCContext.getSCDisplayWithMouse()
            if display != currentDisplay {
                currentDisplay = display
                DispatchQueue.main.async {
                    self?.closeAreaSelector()
                    self?.startAreaSelectorWithDisplayTracking(size: size)
                }
            }
        }
    }
    ...
    func showAreaSelector(size: NSSize, noPanel: Bool = false) {
        ...
        screenshotWindow.makeKeyAndOrderFront(self)
