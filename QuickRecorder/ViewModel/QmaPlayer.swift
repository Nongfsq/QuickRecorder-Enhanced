...
class AudioPlayerManager: ObservableObject {
    ...
    private func renderMixedAudio(to url: URL) throws {
        ...
        engine.stop()
        var manualRenderingEnabled = false
        defer {
        ...
        // exports can be mono. Render in the output file's processing format
        // so AVAudioFile receives exactly the channel count it expects.
        try engine.enableManualRenderingMode(
            .offline,
            format: outputFile.processingFormat,
            maximumFrameCount: 4096
        )
        manualRenderingEnabled = true
        try engine.start()
        ...
        while engine.manualRenderingSampleTime < duration {
            ...
            case .insufficientDataFromInputNode, .cannotDoInCurrentContext:
                ...
                if stalledRenderAttempts > 1000 {
                    throw NSError(domain: "QMAExportError", code: 2, userInfo: [NSLocalizedDescriptionKey: "Offline audio rendering made no progress."])
                }
            case .error:
                throw NSError(domain: "QMAExportError", code: 3, userInfo: [NSLocalizedDescriptionKey: "Offline audio rendering failed."])
            @unknown default:
                throw NSError(domain: "QMAExportError", code: 4, userInfo: [NSLocalizedDescriptionKey: "Offline audio rendering returned an unknown status."])
            }
        }
    }
...
extension UTType {
    static let qma = UTType(exportedAs: "com.nongfsq.QuickRecorderEnhanced.qma")
}
