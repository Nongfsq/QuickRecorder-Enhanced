...
struct PreviewView: View {
                        ...
                        Button(action: {
                            ...
                            ZStack {
                                Image(systemName: "circle.fill")
                                    ...
                                    .opacity(0.5)
                                Image(systemName: isArchiveSupported ? "archivebox.circle" : "play.circle")
                                    .font(.system(size: 50))
    ...
    private var isArchiveSupported: Bool {
        AV1ArchiveCommandBuilder.isSupportedSource(filePath.url)
    }
    ...
    private func openArchiveStatus() {
        let url = filePath.url
        if ArchiveCompressionService.shared.job(for: url) == nil {
            ArchiveCompressionService.shared.startArchive(for: url)
        }
        AppDelegate.shared.createNewWindow(view: ArchiveJobView(sourceURL: url), title: "AV1 Archive".local, random: false, only: false)
    }
