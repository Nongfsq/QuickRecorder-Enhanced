...
var keyMonitor: Any?
var areaSelectorMouseMonitor: Any?
var isClosingAreaSelector = false
...
class AppDelegate: NSObject, NSApplicationDelegate, SCStreamDelegate, SCStreamOutput, AVCaptureVideoDataOutputSampleBufferDelegate  {
    ...
    @AppStorage("microphoneNoiseReduction") var microphoneNoiseReduction: Bool = false
    ...
    @AppStorage("videoBitrate")     var videoBitrate: VideoBitrate = .auto
    @AppStorage("adaptiveVFR")      var adaptiveVFR: Bool = false
    ...
    @AppStorage("audioChannels")    var audioChannels: AudioChannels = .stereo
    ...
    func applicationWillFinishLaunching(_ notification: Notification) {
        ...
        registeredDefaults.merge(
            [
                ...
                "archiveAV1GOP": 270,
                "archiveAV1AudioMode": AV1ArchiveAudioMode.aacMono64k.rawValue,
        ...
        KeyboardShortcuts.onKeyDown(for: .startWithArea) {[self] in
            if SCContext.stream != nil { return }
            closeAllWindow()
            startAreaSelectorWithDisplayTracking()
        }
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        closeAllWindow()
        let isArchiveSmoke = ProcessInfo.processInfo.arguments.contains("--quickrecorder-archive-source")
        if isArchiveSmoke {
            runArchiveSmokeFromLaunchArgumentsIfNeeded()
            return
        }
    ...
    private func runArchiveSmokeFromLaunchArgumentsIfNeeded() {
        let arguments = ProcessInfo.processInfo.arguments
        guard let sourceIndex = arguments.firstIndex(of: "--quickrecorder-archive-source"),
              arguments.indices.contains(sourceIndex + 1) else { return }

        let sourceURL = URL(fileURLWithPath: arguments[sourceIndex + 1])
        if arguments.contains("--quickrecorder-install-bundled-ffmpeg-runtime") {
            try? FFmpegRuntimeInstaller.installBundledRuntime()
        } else if arguments.contains("--quickrecorder-install-local-ffmpeg-runtime") {
            try? FFmpegRuntimeInstaller.installFromLocalFFmpeg()
        }
        print("QuickRecorder archive smoke: source=\(sourceURL.path)")

        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            ArchiveCompressionService.shared.startArchive(for: sourceURL)
            if arguments.contains("--quickrecorder-open-archive-window") {
                self.createNewWindow(view: ArchiveJobView(sourceURL: sourceURL), title: "AV1 Archive".local, random: false, only: false)
            }
            if arguments.contains("--quickrecorder-quit-after-archive") {
                ArchiveCompressionService.shared.quitWhenJobFinishes(for: sourceURL)
            }
        }
    }
