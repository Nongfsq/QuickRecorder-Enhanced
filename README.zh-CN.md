...
当前 `v1.7.0-alpha.2` 提供一个**实验性 Universal DMG**。它使用项目固定的自签名
身份，但尚未使用 Developer ID 签名，也没有经过 Apple 公证。因此首次打开时，macOS
可能要求在 Finder 中右键点击 App 并选择“打开”。

这个 Alpha 安装包暂不内置可选的 FFmpeg 运行库；在完成 Universal 打包和运行库声明
前，AV1 归档需要另行安装兼容的 FFmpeg。
...
每一个正式发布到 Release 页面的版本都必须附带 DMG；如果 DMG 尚未准备好，该版本应
保持 Draft。Alpha DMG 可以明确标注为“自签名、未公证”并供测试使用。面向普通用户的
正式安装包仍需完成 Developer ID 签名、Apple 公证、运行库许可证合规和本项目自己的
更新密钥。
