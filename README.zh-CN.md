...
当前 `v1.7.0-alpha.2` 是一个**源码检查点**，不提供 DMG。公开安装包仍需完成
Developer ID 签名、Apple 公证、可复现的 Universal FFmpeg 打包、完整运行库声明，
以及本项目自己的更新源。

较早的 `v1.7.0-alpha.1` Release 保留了明确标注为实验性、自签名且未公证的 DMG，
仅供历史测试使用，不代表当前支持的安装方式。
